...
class UpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'phone' => [
                'required',
                'string',
                Rule::unique('schools', 'phone')->ignore($this->route('id'))->whereNull('deleted_at'),
            ],
            'email' => [
                'required',
                'string',
                'email',
                Rule::unique('schools', 'email')->ignore($this->route('id'))->whereNull('deleted_at'),
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'email.required' => "email polya ma'jbu'riy",
            'email.email' => "email qate formatda",
